...
(
         ...
         */
        private val window: StringWindow) : Splitter<JsonStreamEvent, JsonChunkMeta> {
    ...
    override fun onEvent(event: JsonStreamEvent): Splitter.Result<JsonChunkMeta>? {
        when (event.event) {
            JsonEvent.START_OBJECT -> {
                if (mark == -1 && inArray.peek() === java.lang.Boolean.TRUE) {
            ...
            JsonEvent.START_ARRAY -> if (mark == -1) {
     ...
     */
    protected open fun makeResult(pos: Int): Splitter.Result<JsonChunkMeta> {
        resultsCreated = true
        val chunk = window.getChars(mark, pos)
        window.advanceTo(pos)
        val meta = JsonChunkMeta(lastFieldName, 0, Utf8.encodedLength(chunk))
        return Splitter.Result(chunk, meta)
    }
}
