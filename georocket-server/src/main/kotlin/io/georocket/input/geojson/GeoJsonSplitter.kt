 ...
 */
(window: StringWindow) : JsonSplitter(window) {
    /**
     ...
     */
    private var highestTypeLevel = Integer.MAX_VALUE
    ...
    override fun onEvent(event: JsonStreamEvent): Splitter.Result<JsonChunkMeta>? {
        ...
        var r: Splitter.Result<JsonChunkMeta>? = super.onEvent(event)

        when (event.event) {
            JsonEvent.FIELD_NAME -> if (mark == -1 || markedLevel == inArray.size + insideLevel - 1) {
        ...
        if (r != null) {
            r = Splitter.Result(r!!.chunk,
                    GeoJsonChunkMeta(lastType, r!!.meta))
        }

        return r
    }

    override fun makeResult(pos: Int): Splitter.Result<JsonChunkMeta> {
        ...
        return if (lastFieldName == null || lastFieldName == "features" ||
                lastFieldName == "geometries") {
